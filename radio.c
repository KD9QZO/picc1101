...
void int_packet(void) {
	...
	if (p_radio_int_data->mode == RADIOMODE_RX) {
		if (int_line) {
			verbprintf(3, "GDO0 RX rising edge (%d,%d)\n", p_radio_int_data->packet_receive, p_radio_int_data->packet_send);
			...
			verbprintf(3, "GDO0 RX falling edge (%d,%d): %d bytes remaining\n", p_radio_int_data->packet_receive, p_radio_int_data->packet_send, p_radio_int_data->bytes_remaining);
...
void print_received_packet(int verbose_min) {
	...
	verbprintf(verbose_min, "RSSI: %.2f dBm. LQI=%d. CRC=%d\n", rssi_dbm(rssi_dec), 0x7F - (crc_lqi & 0x7F), ((crc_lqi & PI_CCxxx0_CRC_OK) >> 7));
}
...
int print_radio_status(spi_parms_t *spi_parms) {
	...
	if (ret != 0) {
		fprintf(stderr, "RADIO: read status register 0x%02X failed\n", reg_index - 1);
		return (ret);
	}
...
uint32_t radio_receive_packet(spi_parms_t *spi_parms, arguments_t *arguments, uint8_t *packet) {
	uint8_t crc;
	uint8_t block_countdown;
	uint8_t block_count = 0;
	uint32_t packet_size = 0;
	uint32_t timeout;
	uint32_t timeout_value = (arguments->packet_length < 32 ? 16 : arguments->packet_length / 2); // timeout value in bocks of 4 2-FSK bytes
